...
module.exports = function (Cam) {
	 ...
	 * @param {object} [options]
	 * @param {string} [options.token] {@link Cam#activeSource.profileToken}
	 * @param {Cam~GetImagingStatusCallback} callback
	 */
	Cam.prototype.getImagingStatus = function (options, callback) {
		...
		this._request({
			...
			if (!xml.includes('xmlns:sc') || !xml.includes('xmlns:s') || !xml.includes('xmlns:tt') || !xml.includes('xmlns:timg')){
				xml = xml.replace('Envelope', 'Envelope xmlns:sc="http://www.w3.org/2003/05/soap-encoding" xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:tt="http://www.onvif.org/ver10/schema" xmlns:timg="http://www.onvif.org/ver20/imaging/wsdl"')
			}
	...
	/**
	 * Get the ImagingOptions for the requested VideoSource (default - the activeSource)
	 * @param {object} [options]
	 * @param {string} [options.token] {@link Cam#activeSource.profileToken}
	 * @param {Cam~GetImagingOptionsCallback} callback
	 */
	Cam.prototype.getImagingOptions = function (options, callback) {
		if (typeof callback === 'undefined') {
			callback = options;
			options = {};
		}
	...
	* @property {number} focus.farLimit
	...
	/**
	 * Set the Focus for the requested VideoSource (default - the activeSource)
	 * @param { distance } options
	 * @param callback
	 */
	Cam.prototype.setFocusDistance = function (options, callback) {
		this._request({
			service: 'imaging'
			, body: this._envelopeHeader() +
			...
			, body: this._envelopeHeader() +
				...
				(
					options.focus ?
						(
							'<Focus xmlns="http://www.onvif.org/ver10/schema">' +
							(options.focus.distance ?
								(
									'<Relative xmlns="http://www.onvif.org/ver10/schema"><Distance>' + options.focus.distance + '</Distance></Relative>'
								) : ''
							) +
							(
